...
/**
 * This class exists to convert a String sequence into the rules for a poker hand. The
 * parseRuleSequence method handles the conversion and tests against a given hand.
 */
public class Converter {

  //CONSTANTS
  private static final int FACE_CARD_LOWEST_VALUE = 11;
   ...
   *
   * @param ruleSequence A string to represent the rules of a poker hand. It should be a sequence of
   * patterns separated by a semicolon. If multiple patterns are provided the hand will only return
   * true if it matches all of the patterns. Cards are removed upon a match so care must be taken to
   * put the more complex patterns first, for example, a three of a kind will evaluate as a pair if
   * the pattern is run first. Within each sequence, pattern rules are separated by commas. Each
   * rule contains two characters, the first character representing the rank of the hand and the
   * second character representing the suit. Hands are sorted by rank upon evaluation, so there is
   * limited flexibility with suit rules. Currently only a flush is supported. The rules for ranks
   * are as follows:
   * <ul>
   * <li>
   ...
   * </li>
   * <li>
   * F - pattern rank is a face card - an Ace or a card higher than rank 11 (Jack)
   * </li>
   * <li>
   ...
   * </li>
   * </ul>
   * @param playerHand The hand of cards to be evaluated against the ruleSequence.
   * @return true if the hand matches the sequence, false if there is a failure on any of the
   * patterns.
   */
